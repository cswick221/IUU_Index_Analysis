...
all_index_df <- skylight_df_final %>% 
  ...
  select(-treatment_year, -sessions, -no_active_weeks)

na_counts <- colSums(is.na(all_index_df))

all_indicators <- as.data.frame(unique(names(all_index_df[6:57])))
...
scaled_columns <- scale(all_index_df[, 6:57], center = TRUE, scale = TRUE)
...
scaled_all_index_df <- scaled_all_index_df[, c(1, 3:5, 2, 6:57)] %>% 
  clean_names()
