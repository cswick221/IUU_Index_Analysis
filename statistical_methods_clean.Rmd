...
wb_gdp_df <- read_csv(here("data", "World Bank GDP.csv")) %>%
  clean_names() %>% 
  rename(country=country_name) %>% 
  rename_all(~ gsub("x(\\d+)_yr\\1", "\\1", .)) %>% 
...
countries_to_exclude <- 
  c("Montenegro", "Cook Islands", "Falkland Islands", "Pitcairn Islands", "Réunion", "Antarctica")
...
scaled_columns <- scale(all_index_df[, 6:49], center = TRUE, scale = TRUE)
...
scaled_all_index_df <- scaled_all_index_df[, c(1, 3:5, 2, 6:48)] %>% 
  na.omit()
...
```{r}
glm_1 <- glm(skylight_treatment ~ ., data = scaled_all_index_df[, 5:49], family = binomial)
...
pca_result <- prcomp(scaled_all_index_df[, 6:48], scale. = TRUE)
...
pca_summary_df <- data.frame(
  ...
  Proportion_of_Variance_Explained = prop_variance,
  Cumulative_Proportion_of_Variance_Explained = cumulative_prop_variance
)
...
k <- 5  # You can adjust this value based on your specific problem and requirements

# Perform k-means clustering
kmeans_result <- kmeans(scaled_all_index_df[, 6:48], centers = k)
...
```{r}
t_test_results <- data.frame(Indicator = numeric(), 
                              Mean_Group_0 = numeric(), 
                              Mean_Group_1 = numeric(), 
                              p_value = numeric(),
                              stringsAsFactors = FALSE)

# Iterate over each indicator column
for (col in names(scaled_all_index_df)[6:49]) {
  # Perform two-sample t-test for the indicator column
  t_test <- t.test(scaled_all_index_df[[col]][scaled_all_index_df$skylight_treatment == 0], 
                   scaled_all_index_df[[col]][scaled_all_index_df$skylight_treatment == 1])
  
  # Store the results in the dataframe
  t_test_results <- rbind(t_test_results, 
                          data.frame(Indicator = col, 
                                     Mean_Group_0 = mean(scaled_all_index_df[[col]][scaled_all_index_df$skylight_treatment == 0]), 
                                     Mean_Group_1 = mean(scaled_all_index_df[[col]][scaled_all_index_df$skylight_treatment == 1]), 
...
```{r}
t_test_result <- t.test(scaled_all_index_df$government_effectiveness_estimate[scaled_all_index_df$skylight_treatment == 0], 
                        scaled_all_index_df$government_effectiveness_estimate[scaled_all_index_df$skylight_treatment == 1])
