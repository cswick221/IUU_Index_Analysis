...
### Skylight data 
## why do we need engagements and skylight deal dates? are we doing something with this? 
```{r}
...
iuufr_index_final <- iuu_index_scores %>% 
  ...
  mutate(year = as.Date(year)) %>%  #somehow this is turning all of the years to 1975?
  group_by(country) %>%
  arrange(year) %>%
  select(-year) %>% #why are we getting rid of year? how are we going to join later?
  fill(everything()) %>% ## I don't understand what this code is doing
  slice(n()) ## I don't understand what this code is doing
...
wb_gov_final_df <- wb_gov_index_df %>%
  group_by(country) %>%
  filter(year == max(year, na.rm = TRUE)) %>% #does this line remove NAs? # Filter for the maximum year value
  ungroup() %>%
  select(-year) %>% #why are we removing the year?
  fill(everything())
...
wb_gdp_pc_final <- wb_gdp_pc %>% 
  ...
  group_by(country) %>%
  filter(!is.na(gdp_per_capita)) %>% #removing NA rows, why are we doing this?
...
fish_index_final <- fish_id_clean %>% 
  group_by(country) %>%
  filter(!is.na(aquaculture_production_metric_tons)) %>% #removing NA rows 
  slice_max(order_by = year, n = 1) %>%
  ungroup() %>%
  distinct(country, .keep_all = TRUE)
