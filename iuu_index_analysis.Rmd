...
deal_date_test <- skylight_deals %>% 
  subset(select = ctk)

deal_date_test <- deal_date_test %>% 
...
## 66 countries as deploying or engaged
```


countries listed as "dormant" but active in analytics data:
micronesia --> 17/2 sessions
malaysia --> 8/53. 
papua new guinea --> 64. 
samoa --> 14/5. 
thailand--> 376. 
...
############## testing above code with deal_date_test 
dind_test <- dind_df %>% 
  left_join(deal_date_test %>% 
    select(deal_country, treatment_year), 
    by = c("skylight_countries" = "deal_country")) 

####### you end up with a bunch of countries having 1 under post treatment that aren't using Skylight, I wonder if this is something we need to worry about in the sense that a country might have been using Skylight in 2019 or 2021 but doesn't now 
...
```{r}
trying_df <- country_engagements_total %>% 
  filter(grepl("Gabon", country))
